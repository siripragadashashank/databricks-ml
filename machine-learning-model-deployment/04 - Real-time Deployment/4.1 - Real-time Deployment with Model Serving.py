...
# MAGIC
# MAGIC * To run this notebook, you need to use one of the following Databricks runtime(s): **{{supported_dbrs}}**
# MAGIC
...
# MAGIC
# MAGIC For this demonstration, we will use a fictional dataset from a Telecom Company, which includes customer information. This dataset encompasses **customer demographics**, including internet subscription details such as subscription plans, monthly chardes and payment methods.
# MAGIC
...
# MAGIC
# MAGIC Before we start model deployment, we need to fit and register a model. In this demo, **we will log models to Unity Catalog**, which means first we need to setup the **MLflow Model Registery URI**.
...
# MAGIC
# MAGIC Let's serve the two models we logged in the previous step using Model Serving. Model Serving supports enpoint management via the UI and the API. 
# MAGIC
# MAGIC Below you will find instructions for using the UI and it is simplier method compared to the API. **In this demo, we will use the API to configure and create the endpoint**.
# MAGIC
...
# MAGIC   
# MAGIC     f. *[OPTIONAL]* to deploy another model (e.g. for A/B testing) click on **+Add Served Entity** and fill the abovementioned details.
# MAGIC   
...
endpoint_name = "db_academy_ml_endpoint"
...
# MAGIC   
# MAGIC   * `Sync mode`:  Select **`Snapshot`** for Sync mode. Please refer to the documenation for more details about available options.
# MAGIC   
...
# MAGIC
# MAGIC **💡 Note:** The Model Serving **endpoint configuration and creation process is the same for serving models with or wtihout feature tables**. The registered model metadata handles feature lookup during inference.
...
fs_endpoint_name = "db_academy_ml_fs_endpoint"
