...
# MAGIC
# MAGIC Now that our model is ready in model registery, we can use it for inference. In this section we will use the model for inference directly on a spark dataframe, which called **batch inference**.
...
# MAGIC
# MAGIC Loading a model from UC-based model registery is done by getting a model using **alias** and **version**. 
# MAGIC
