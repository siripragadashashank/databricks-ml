...
# MAGIC
# MAGIC **Note:** This section is not the main focus of this course. We are just repeating the model development and registeration process here.
...
# MAGIC
# MAGIC **Note: If you want to learn more about DLT please check out `Data Engineerign with Databricks (Data Pipeline with Delta Live Tables).**
# MAGIC
...
# MAGIC
# MAGIC 1. For `Pipeline name` enter **Demo3.1-Pipeline Deployment**.
# MAGIC
# MAGIC 1. For `Pipeline mode` select **Triggered**.
# MAGIC
# MAGIC 1. For `Paths` select the notebook **3.1.b - Inference Pipeline**.
# MAGIC
# MAGIC 1. For Destination catalog and schema, **select the Catalog and Schema that is created for you in the beginning of this notebook**.
# MAGIC
# MAGIC 1. For compute section, **select a compute resource configuration that supports DLT.** *If you have issues with DLT supported compute resources, you will need to contact your workspace admin.*
# MAGIC
# MAGIC 1. For **Advanced** section, add these two configurations using the values from previous cell's output;
# MAGIC     * `mlpipeline.bronze_dataset_path`
# MAGIC     * `mlpipeline.model_name`  
# MAGIC
# MAGIC 1. For **Advanced** section, **select *Preview* Channel**.
